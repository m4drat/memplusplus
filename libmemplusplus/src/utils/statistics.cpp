...
namespace mpp {
    namespace utils {
        ...
        std::string Statistics::FormattedSize(std::size_t t_bytes, uint32_t t_decimals) 
        {
            ...
            ss << std::setprecision(t_decimals) << std::fixed << (t_bytes / std::pow(k, i)) << " " << sizes.at(i);
        ...
        std::ostream& Statistics::DumpArenasGeneralStats(std::ostream& t_out)
        {
            ...
            for (const auto& arenaStats : m_ArenasStats) {
                t_out << "  -----------------  Arena: " << arenaIdx << "  -----------------  " << std::endl;
                t_out << "MPP - Total amount of allocated memory inside arena : " << FormattedSize(arenaStats->totalAllocated) << std::endl;
                t_out << "MPP - Total amount of freed memory inside arena     : " << FormattedSize(arenaStats->totalFreed) << std::endl;
                ...
                if (arenaStats->biggestAllocation == 0) {
                    t_out << "MPP - Biggest allocation size                       : \"No allocations have been made yet\"" << std::endl;
                } else {
                    t_out << "MPP - Biggest allocation size                       : " << FormattedSize(arenaStats->biggestAllocation) << std::endl;
                }
                if (arenaStats->smallestAllocation == std::numeric_limits<std::size_t>::max()) {
                    t_out << "MPP - Smallest allocation size                      : \"No allocations have been made yet\"" << std::endl;
                } else {
                    t_out << "MPP - Smallest allocation size                      : " << FormattedSize(arenaStats->smallestAllocation) << std::endl;
                }
                t_out << "MPP - Full size of arena                            : " << FormattedSize(arenaStats->fullArenaSize) << std::endl;
                t_out << "MPP - Arena was allocated for big chunk             : " << (arenaStats->bigArena ? "true" : "false") << std::endl;
                t_out << "MPP - Arena was allocated by GC                     : " << (arenaStats->gcCreatedArena ? "true" : "false") << std::endl;
                t_out << std::endl;
                arenaIdx++;
            }
            return t_out;
        ...
        std::ostream& Statistics::DumpGcStats(std::ostream& t_out)
        {
            ...
            for (const auto& gcStats : m_GcCyclesStats) {
                t_out << "  -----------------  Cycle: " << cycleIdx << "  -----------------  " << std::endl;
                t_out << "GC - Time wasted inside   GC::Collect() : " << gcStats->timeWasted.count() << " ms" << std::endl;
                t_out << "GC - Memory cleaned after GC::Collect() : " << FormattedSize(gcStats->memoryCleaned) << std::endl;
                t_out << "GC - Total size of active objects       : " << FormattedSize(gcStats->activeObjectsTotalSize) << std::endl;
                t_out << std::endl;
                cycleIdx++;
            }
            return t_out;
        ...
        std::ostream& Statistics::DumpStats(std::ostream& t_out, bool t_dumpActiveArenas, bool t_DumpFreedChunks, bool t_DumpInUseChunks)
        {
            t_out << "+============= STATISTICS DUMP START =============+" << std::endl;

            t_out << "MPP: MIN_CHUNK_SIZE     : " << MemoryManager::g_MIN_CHUNK_SIZE << " bytes" << std::endl;
            t_out << "MPP: CHUNK_HEADER_SIZE  : " << sizeof(Chunk::ChunkHeader) << " bytes" << std::endl;
            t_out << "MPP: DEFAULT_ARENA_SIZE : " << FormattedSize(MemoryManager::g_DEFAULT_ARENA_SIZE)
                  << std::endl;
            t_out << "MPP: PAGE_SIZE          : " << FormattedSize(MemoryManager::g_PAGE_SIZE) << std::endl;
