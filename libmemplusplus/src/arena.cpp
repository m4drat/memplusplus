...
Chunk* Arena::MergeNeighborsChunks(Chunk* t_chunk)
{
    ...
    */
    ...
    First case:
        ...
        3. DT
    */
    if (((topChunk == nullptr) && (reinterpret_cast<void*>(reinterpret_cast<std::size_t>(t_chunk) +
                                                           t_chunk->GetSize()) == end)) ||
                               ...
                               bool t_DumpInUseChunks)
{
    ...
    t_out << "MPP - Amount of free memory      : "
          << utils::Statistics::FormattedSize(t_arena->size - t_arena->m_CurrentlyAllocatedSpace)
