...
namespace mpp {
    ...
    bool GC::Collect()
    {
        ...
        std::size_t godArenaSize = MemoryAllocator::Align((layoutedData.second < g_DEFAULT_ARENA_SIZE) ? 
            g_DEFAULT_ARENA_SIZE : layoutedData.second, g_PAGE_SIZE);
        ...
        if (godArenaSize - layoutedData.second == 0)
        {
            ...
            Chunk* topChunk = Chunk::ConstructChunk(currPtr,
                                  prevSize,
                                  godArenaSize - layoutedData.second,
                                  1, 1);
